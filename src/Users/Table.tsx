...
export default function UserTable() {
    ...
    const [user, setUser] = useState<User>({ username: "", password: "", firstName: "",
        lastName: "", role: "USER"
    });
    ...
    const selectUser = async (user: User) => {
        try {
            const u = user._id ? await client.findUserById(user._id) : null;
            setUser(u);
        } catch (err) {
            console.log(err);
        }
    ...
    return (
        <div>
            <select
                ...
                value={role || "USER"}
                className="form-select w-25 float-end m-2"
            >
